...
// You can edit this contract as much as you want. A template is provided here and you can change the function names and implement anything else you want, but the basic tasks mentioned here should be accomplished.
    ...
    function getCompoundInterest(uint256 principle, uint rate, uint time) public pure returns(uint256) {
        ...
        principle=principle*1e10;
        while(time>0){
            principle+=mulDiv(principle,rate,100);
            time--;
        }
        return principle/1e10;
    }
    ...
    function settleDues(address creditor) public isOwner returns(bool){
        ...
        bool x=MetaCoin.sendCoin(creditor,loans[creditor],owner);
        if(x)
        {
            loans[creditor]=0;
            return x;
    ...
    // They take in the address of a creditor as arguments. viewDues returns a uint256 corresponding to the due amount, and does not modify any state variables. settleDues returns a bool, true if the dues were settled and false otherwise.
    // Remember to set the the pending loan to 0 after settling the dues.
